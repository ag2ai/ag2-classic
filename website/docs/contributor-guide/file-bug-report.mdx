...
!!! important "AG2 Classic is in maintenance mode"

    We only fix **security issues** and **critical bugs** in existing behaviour — no new features or enhancements. For feature ideas, please use the actively developed [AG2 repository](https://github.com/ag2ai/ag2){.external-link target="_blank"}.

    For a **security vulnerability**, do **not** open a public issue — follow the private process in the repository's `SECURITY.md`.

When you submit an issue to [GitHub](https://github.com/ag2ai/ag2-classic/issues){.external-link target="_blank"}, please do your best to
