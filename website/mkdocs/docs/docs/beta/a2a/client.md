...
This composes naturally with several remotes — give each `#!python as_tool()` a distinct `name=` and let the local LLM route by capability. See [Subagents](../subagents.md) for the general `#!python as_tool()` semantics.
